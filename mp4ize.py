...
#        p = Popen ([cmd, '-i', f], stdout=PIPE, stderr=PIPE)
#        output = p.stderr.read()
        #output = p.communicate()[1] #read in sterr
        ...
        if not m:
            ...
            m = pattern.search(output)
            if m:
                #print 'got match test %s ' % m.group(0)
                w = float(m.group(1))
                h = float(m.group(2))
...
#                 sys.exit(1)            
        ...
        #print 'asdf', returncode, output
        
        #output = p.communicate() #read in stout    
        if returncode == 0:
            #time = verify_output(t)
            #print 'time=', time
            #print 'time=', verify_output(output)   
        ...
        if readx:
            ...
            if tmp_time_f:
                ...
                if duration:
	                ...
	                #printf "%2g ", percent
                    #sys.stderr.write(' %2g Completed  [%s] \r' % (percent, time_f))
...
def verify_output(output):
    time_f = 0
    #pattern = re.compile(r'time=(.*) bitrate')    
    ...
    if type(output).__name__ == 'tuple':            
        ...
        #pattern = re.compile( r'time=(.*) [\w+]' )                    
        #pattern = re.compile( r'time=([^\s]+) bitrate' )                 
        #pattern = re.compile( r'time=([^\s]+)' )                 
        pattern = re.compile( r'time=([^\s]+)' )                                 
...
#    usage = "%prog [options]"
    ...
    parser = argparse.ArgumentParser(description='encode videos to mp4 for an iPod or an iPhone')
...
#    parser.add_argument('input', metavar=('file1.avi', 'file2.mpg file3.asf'), type=str, nargs='+', 
#                       help='list of file names to process')

    parser.add_argument('input', metavar=('file'), type=str, nargs='+', 
                       help='list of file names to process')

    # setting defaults creates a value in args, otherwise the value passed will be None
    parser.add_argument('-a', '--audio', metavar='RATE', type=str, nargs=1, default=DEFAULT_AUDIO_BITRATE,
                       help='override default audio bitrate (%(default)s)' )    
    parser.add_argument('-b', '--video', metavar='RATE', type=str, nargs=1, default=DEFAULT_VIDEO_BITRATE,
                       help='override default video bitrate (%(default)s)' )    
    parser.add_argument('-v', '--verbose', action='store_true')   

    parser.add_argument('-w', '--width', type=int, nargs=1, default=int(IPOD_WIDTH),
